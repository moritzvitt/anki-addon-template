...
GLOBAL_STYLING_PROVIDER_ATTR = "_moritz_global_styling_provider"
GLOBAL_STYLING_THEME_CHOICE_ATTR = "_moritz_global_styling_theme_choice"
GLOBAL_STYLING_ADDON_PACKAGE = "moritz_global_styling"
GLOBAL_STYLING_ADDON_NAME = "Global Styling"
...
THEME_PRESETS: tuple[ThemePreset, ...] = (
    ThemePreset(
        theme_id="off",
        label="Off",
        description="Disable Global Styling so each add-on falls back to its own built-in styling.",
    ),
    ThemePreset(
        theme_id="clean",
        label="Clean Anki",
        description="Light-touch styling that keeps the dialogs close to Anki's native look.",
    ),
    ThemePreset(
        theme_id="heatmap",
        label="Heatmap / Goal Bar",
        description="Warm surfaces and green accents inspired by Review Heatmap and your goal bar designs.",
    ),
    ThemePreset(
        theme_id="modern",
        label="Modern Cards",
        description="A cleaner card-like look with stronger contrast, softer shadows, and a more contemporary accent color.",
    ),
)

DEFAULT_THEME_ID = "clean"


def theme_presets() -> tuple[ThemePreset, ...]:
    return THEME_PRESETS


def normalize_theme_id(theme_id: str | None) -> str:
    if any(preset.theme_id == theme_id for preset in THEME_PRESETS):
        return str(theme_id)
...
def theme_label(theme_id: str | None) -> str:
    ...
    return next(preset.label for preset in THEME_PRESETS if preset.theme_id == normalized)


def is_theme_enabled(theme_id: str | None) -> bool:
    return normalize_theme_id(theme_id) != "off"
...
def theme_tokens(theme_id: str | None, palette: QPalette) -> dict[str, str]:
    normalized = normalize_theme_id(theme_id)
...
def build_dialog_stylesheet(theme_id: str | None, palette: QPalette) -> str:
    if not is_theme_enabled(theme_id):
        return ""
    colors = theme_tokens(theme_id, palette)
    return f"""
QDialog {{
...
def build_web_theme_css(theme_id: str | None, palette: QPalette, selector: str) -> str:
    if not is_theme_enabled(theme_id):
        return ""
    colors = theme_tokens(theme_id, palette)
    return f"""
